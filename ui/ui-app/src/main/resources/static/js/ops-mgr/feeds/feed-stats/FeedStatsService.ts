...
export class FeedStatsService {
    ...
    fetchProcessorStatistics(minTime: any, maxTime: any) {
        ...
        if (ObjectUtils.isUndefined(minTime)) {
            minTime = this.fromMillis;
        }
        if (ObjectUtils.isUndefined(maxTime)) {
            maxTime = this.toMillis;
        }
        return new Promise((resolve, reject) => {
            this.provenanceEventStatsService.getFeedProcessorDuration(this.feedName, minTime, maxTime).then((response: any) => {
                var processorStatsContainer = response;
                ...
                var lastSummaryStats = CloneUtil.deepCopy(this.summaryStatistics);
                ...
                resolve(this.processorStatistics);
    ...
    fetchFeedTimeSeriesData() {
        ...
        return new Promise((resolve, reject) => {
            this.provenanceEventStatsService.getFeedStatisticsOverTime(this.feedName, this.fromMillis, this.toMillis).then((response: any) => {

                var statsContainer = response;

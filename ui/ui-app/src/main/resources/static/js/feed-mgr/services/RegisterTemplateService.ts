...
export class RegisterTemplateService {
    constructor (private $http:any, private $q:any, private $mdDialog:any, private RestUrlService:any
         ...
         * @param processor
         */
        function setRenderTemplateForProcessor(processor:any, mode:any) {
        ...
        var data:any = {
            ...
            isSelectedProperty: function (property:any) {
            ...
            getSelectedProperties: function () {
                ...
                angular.forEach(self.model.inputProperties, function (property:any) {
                ...
                angular.forEach(self.model.additionalProperties, function (property:any) {
                    if (data.isSelectedProperty(property)) {
                        selectedProperties.push(property);
                        if (property.processor && property.processor.topIndex != undefined) {
                            delete property.processor.topIndex;
            ...
            sortPropertiesForDisplay: function (properties:any) {
                var propertiesAndProcessors:any = {properties: [], processors: []};
            ...
            fetchConfigurationProperties: function (successFn:any, errorFn:any) {

                var self = this;
                if (self.configurationProperties.length == 0) {
                    var _successFn = function (response:any) {
                        self.configurationProperties = response.data;
                        angular.forEach(response.data, function (value:any, key:any) {
            ...
            fetchMetadataProperties: function (successFn:any, errorFn:any) {
                var self = this;
                if (self.metadataProperties.length == 0) {
                    var _successFn = function (response:any) {
                        self.metadataProperties = response.data;
                        angular.forEach(response.data, function (annotatedProperty:any, i:any) {
            ...
            replaceAll: function (str:any, find:any, replace:any) {
                return str.replace(new RegExp(escapeRegExp(find), 'g'), replace);
            },
            deriveExpression: function (expression:any, configOnly:any) {
                var self = this;
                var replaced = false;
                if (expression != null && expression != '') {
                    var variables = expression.match(/\$\{(.*?)\}/gi);
                    if (variables && variables.length) {
                        angular.forEach(variables, function (variable:any) {
            ...
            getCodeMirrorTypes: function () {
                ...
                if (this.codemirrorTypes == null) {
                    var successFn = function (response:any) {
                        ...
                        angular.forEach(self.codemirrorTypes, function (label:any, type:any) {
            ...
            isRenderPropertyWithCodeMirror: function (property:any) {
             ...
             * @returns {boolean}
             */
            isCustomPropertyRendering: function (key:any) {
                var self = this;
            ...
            removeNonUserEditableProperties: function (processorArray:any, keepProcessorIfEmpty:any) {
                ...
                _.each(processorArray, function (processor:any, i:any) {
                    processor.allProperties = processor.properties;

                    var validProperties = _.reject(processor.properties, function (property:any) {
             ...
             * @param model
             */
            setProcessorRenderTemplateUrl : function(model:any, mode:any){
                _.each(model.inputProcessors, function (processor:any) {
                    processor.feedPropertiesUrl = null;
                    //ensure the processorId attr is set
                    processor.processorId = processor.id
                    setRenderTemplateForProcessor(processor, mode);
                });
                _.each(model.nonInputProcessors, function (processor:any) {
                    processor.feedPropertiesUrl = null;
                    //ensure the processorId attr is set
                    processor.processorId = processor.id
                    setRenderTemplateForProcessor(processor, mode);
             ...
             * @param template
             */
            initializeProperties: function (template:any, mode:any, feedProperties:any) {
                //get the saved properties

                var savedProperties = {};
                if (feedProperties) {
                    _.each(feedProperties, function (property:any) {
                ...
                function updateProperties(processor:any, properties:any) {

                    _.each(properties, function (property:any) {
                ...
                _.each(template.inputProcessors, function (processor:any) {
                    //ensure the processorId attr is set
                    processor.processorId = processor.id
                    updateProperties(processor, processor.properties)
                    setRenderTemplateForProcessor(processor, mode);
                });
                _.each(template.nonInputProcessors, function (processor:any) {
                    //ensure the processorId attr is set
                    processor.processorId = processor.id
                    updateProperties(processor, processor.properties)
                    setRenderTemplateForProcessor(processor, mode);
            ...
            disableTemplate: function (templateId:any) {
                var self = this;
                var promise = $http.post(RestUrlService.DISABLE_REGISTERED_TEMPLATE_URL(templateId)).then(function (response:any) {
             ...
             * @returns {*}
             */
            enableTemplate: function (templateId:any) {
                var self = this;
                var promise = $http.post(RestUrlService.ENABLE_REGISTERED_TEMPLATE_URL(templateId)).then(function (response:any) {
                    self.model.state = response.data.state
                    if (self.model.state == 'ENABLED') {
                        self.model.stateIcon = 'check_circle'
                    }
            ...
            deleteTemplate: function (templateId:any) {
                var deferred = $q.defer();
                $http.delete(RestUrlService.DELETE_REGISTERED_TEMPLATE_URL(templateId)).then(function (response:any) {
            ...
            getTemplateProcessorDatasourceDefinitions: function (nifiTemplateId:any, inputPortIds:any) {
                var deferred = $q.defer();
                if (nifiTemplateId != null) {
                    $http.get(RestUrlService.TEMPLATE_PROCESSOR_DATASOURCE_DEFINITIONS(nifiTemplateId), {params: {inputPorts: inputPortIds}}).then(function (response:any) {
                        deferred.resolve(response);
                    }, function (response:any) {
                        deferred.reject(response);
                    });
             ...
             */
            getNiFiTemplateFlowInformation: function (nifiTemplateId:any, reusableTemplateConnections:any) {
                var deferred = $q.defer();
                if (nifiTemplateId != null) {
                    //build the request
                    var flowRequest:any = {};
                    ...
                    $http.post(RestUrlService.TEMPLATE_FLOW_INFORMATION(nifiTemplateId), flowRequest).then(function (response:any) {
                        deferred.resolve(response);
                    }, function (response:any) {
                        deferred.reject(response);
                    });
            ...
            accessDeniedDialog:function($filter:any) {
                $mdDialog.show(
                    $mdDialog.alert()
                        .clickOutsideToClose(true)
             ...
             */
            checkTemplateAccess:function(model:any){
                var self = data;
                if(model == undefined){
                    model = self.model;
                }
             ...
             * @returns {*}
             */
            loadTemplateWithProperties: function (registeredTemplateId:any, nifiTemplateId:any, templateName:any) {
                var isValid = true;
                 ...
                 * @param property
                 */
                function assignPropertyRenderType(property:any) {
                    ...
                    if (allowableValues !== undefined && allowableValues !== null && allowableValues.length > 0) {
                        if (allowableValues.length == 2) {
                            var list = _.filter(allowableValues, function (value:any) {
                 ...
                 * @param properties
                 */
                function transformPropertiesToArray(properties:any) {
                    var inputProperties:any = [];
                    var additionalProperties:any = [];
                    var inputProcessors = [];
                    var additionalProcessors = [];
                    angular.forEach(properties, function (property:any, i:any) {
                 ...
                 * @param properties
                 */
                function fixDuplicateProcessorNames(properties:any) {
                        ...
                        if (Object.keys(processorMap).length > 1) {
                            ...
                            _.each(processorMap, function (props, processorId) {
                                if (lastId == null || lastId != processorId) {
                                    idx++;
                                    _.each(props, function (prop:any) {
                 ...
                 */
                function validateTemplateProcessorNames(inputProperties:any, additionalProperties:any) {

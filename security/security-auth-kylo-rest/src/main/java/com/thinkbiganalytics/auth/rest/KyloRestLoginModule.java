...
public class KyloRestLoginModule extends AbstractLoginModule implements LoginModule {
	...
	protected boolean doLogin() throws Exception {
		...
		try { 
			 user = retrieveUser(userConfig); 
				...
				log.debug("Received unauthorized response from Login API for user: {}",
						userConfig.getUsername()); throw new
						CredentialException("The username and password combination do not match."); 
		}
		catch (final ProcessingException e) {
				log.error("Failed to process response from Login API for user: {}",
						userConfig.getUsername(), e); throw new
						FailedLoginException("The login service is unavailable."); 
		} 
		catch (final WebApplicationException e) {
							log.error("Received unexpected response from Login API for user: {}",
									userConfig.getUsername(), e); 
							throw new
									FailedLoginException("The login service is unavailable."); 
		}
						// Parse response 
			if (user == null) {
				...
				throw new AccountNotFoundException("No account exists with the name: " +
								userConfig.getUsername()); 
			}
			else if (!user.isEnabled()) {
				log.debug("User from Login API is disabled: {}", userConfig.getUsername());
					throw new AccountLockedException("The account \"" + userConfig.getUsername()
	...
	private LoginJerseyClientConfig createClientConfig(boolean usePasswords) throws LoginException {
		if (usePasswords) {
			final LoginJerseyClientConfig userConfig = new LoginJerseyClientConfig(config);
			  HttpHeaderCallback userNameheaderCallback = new HttpHeaderCallback("x-preferred-username"); 
			  final String authUser; 
			  final String username; 
			  String password = "";
			  
			  if (loginUser == null) { // Use user's own username and password to accessthe REST API if a loginUser was not provided. 
				  handle(userNameheaderCallback); 
				  authUser = null; 
				  username = userNameheaderCallback.getValues().get(0); 
				  }
			  else { // Using the loginUser to access API so only need the authenticating user's name.
				  handle(userNameheaderCallback); 
				  authUser = userNameheaderCallback.getValues().get(0); 
				  username = loginUser; 
			  }
			userConfig.setUsername(username);
			userConfig.setPassword(password.toCharArray());
	...
	private User retrieveUser(@Nonnull final LoginJerseyClientConfig userConfig) throws LoginException {
		...
		final MultivaluedMap<String, Object> headers = new MultivaluedHashMap<>(Collections.singletonMap("x-preferred-username", userConfig.getUsername()));
		
		return client.getWithHeaders(endpoint, headers, null, User.class);
	}

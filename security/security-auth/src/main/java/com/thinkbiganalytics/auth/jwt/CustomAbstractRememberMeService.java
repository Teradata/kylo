 ...
 * #L%
 */
public abstract class CustomAbstractRememberMeService implements RememberMeServices, InitializingBean, LogoutHandler {
	...
	protected CustomAbstractRememberMeService(String key, UserDetailsService userDetailsService) {
	...
	/**
	 * Template implementation which locates the Spring Security cookie, decodes it
	 * into a delimited array of tokens and submits it to subclasses for processing
	...
	public Authentication autoLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		logger.info("Remember-me cookie detected");

		if (rememberMeCookie.length() == 0) {
			logger.info("Cookie was empty");
		...
		try {
			...
			logger.info("Remember-me cookie accepted");
	 ...
	 *                <tt>AuthenticationDetailsSource</tt> will use this to build
	 *                the details property of the returned object.
	 * @param user    the <tt>UserDetails</tt> loaded from the
	 *                <tt>UserDetailsService</tt>. This will be stored as the
	 *                principal.
	 *
	 ...
	 * "remember me" parameter. If it's present, or if <tt>alwaysRemember</tt> is
	 * set to true, calls <tt>onLoginSucces</tt>.
	 * </p>
	 */
	...
	/**
	 * Allows customization of whether a remember-me login has been requested. The
	 * default is to return true if <tt>alwaysRemember</tt> is set or the configured
	 * parameter name has been included in the request and is set to the value
	 * "true".
	 *
	 * @param request   the request submitted from an interactive login, which may
	 *                  include additional information indicating that a persistent
	 *                  login is desired.
	 * @param parameter the configured remember-me parameter name.
	 *
	 * @return true if the request includes information indicating that a persistent
	 ...
	 * Subclasses should validate the cookie and perform any additional management
	 * required.
	 *
	 * @param cookieTokens the decoded and tokenized cookie value
	 * @param request      the request
	 * @param response     the response, to allow the cookie to be modified if
	 *                     required.
	 * @return the UserDetails for the corresponding user account if the cookie was
	 *         validated successfully.
	 * @throws RememberMeAuthenticationException if the cookie is invalid or the
	 *                                           login is invalid for some other
	 *                                           reason.
	 * @throws UsernameNotFoundException         if the user account corresponding
	 *                                           to the login cookie couldn't be
	 *                                           found (for example if the user has
	 ...
	 *
	 * By default a secure cookie will be used if the connection is secure. You can
	 * set the {@code useSecureCookie} property to {@code false} to override this.
	 * If you set it to {@code true}, the cookie will always be flagged as secure.
	 * If Servlet 3.0 is used, the cookie will be marked as HttpOnly.
	 *
	...
	/**
	 * Sets the name of the parameter which should be checked for to see if a
	 * remember-me has been requested during a login request. This should be the
	 * same name you assign to the checkbox in your login form.
	 *
	 ...
	 * <p>
	 * By default the cookie will be secure if the request is secure. If you only
	 * want to use remember-me over HTTPS (recommended) you should set this property
	 * to {@code true}.
	 *

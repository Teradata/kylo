...
public class NifiPipelineControllerDao implements InitializingBean {
    ...
    public Long findJobExecutionId(Long eventId, String flowfileId) {
        String sql = "SELECT JOB_EXECUTION_ID FROM BATCH_NIFI_JOB WHERE EVENT_ID = ? AND FLOW_FILE_UUID = ? ";
        Long id = null;
        try {
            id = jdbcTemplate.queryForObject(sql, new Object[]{eventId, flowfileId}, new RowMapper<Long>() {
                @Override
                public Long mapRow(ResultSet resultSet, int i) throws SQLException {
                    return resultSet.getLong("JOB_EXECUTION_ID");
                }
            });
    ...
    /**
     * /** CREATE TABLE NIFI_PIPELINE_CONTROLLER_STEP  ( EVENT_ID BIGINT NOT NULL PRIMARY KEY, NIFI_EVENT_ID BIGINT, COMPONENT_ID VARCHAR(255), JOB_EXECUTION_ID BIGINT, STEP_EXECUTION_ID BIGINT )
     * ENGINE=InnoDB; <p> <p> CREATE TABLE NIFI_PIPELINE_CONTROLLER_JOB  ( EVENT_ID BIGINT NOT NULL PRIMARY KEY, NIFI_EVENT_ID BIGINT, FLOW_FILE_UUID VARCHAR(255), FEED_ID BIGINT, FEED_NAME
     * VARCHAR(255), JOB_INSTANCE_ID BIGINT, JOB_EXECUTION_ID BIGINT ) ENGINE=InnoDB;
     */

...
public class ProvenanceEventReceiverDatabaseWriter implements InitializingBean {
    ...
    public Long writeEvent(ProvenanceEventDTO event) {
        ...
        try {
            ...
            String sql = "INSERT INTO " + NIFI_PROVENANCE_EVENT_TABLE + "(EVENT_ID,NIFI_EVENT_ID,EVENT_TYPE,FLOW_FILE_UUID,FLOW_FILE_ENTRY_DATE,EVENT_TIME,EVENT_DURATION,LINEAGE_START_DATE,COMPONENT_ID,COMPONENT_TYPE,DETAILS,PARENT_UUIDS,CHILD_UUIDS,ATTRIBUTES_JSON,JSON, CREATE_DATE)" +
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            Object[] params = new Object[]{id, eventId, event.getEventType().toString(), event.getFlowFileUuid(),
                    ...
                    event.getDetails(), StringUtils.join(event.getParentUuids(), ","), StringUtils.join(event.getChildUuids(), ","), attributesJson, json, new Date(System.currentTimeMillis())};

...
public class NifiEventStatisticsProvider implements ProvenanceEventSummaryStatsProvider {
    ...
    public List<? extends ProvenanceEventSummaryStats> findForFeedGroupedByProcessor(String feedName, TimeFrame timeFrame) {
        DateTime now = DateTime.now();
        return findForFeedGroupedByProcessor(feedName, timeFrame.startTimeRelativeTo(now), now);
    }
    ...
    @Override
    public List<? extends ProvenanceEventSummaryStats> findForFeedGroupedByProcessor(String feedName, DateTime start, DateTime end) {
        QNifiEventSummaryStats stats = QNifiEventSummaryStats.nifiEventSummaryStats;
        JPAQuery
            ...
            Projections.bean(NifiEventSummaryStats.class,
                             ...
                             stats.maxEventTime.max().as("maxEventTime"), stats.minEventTime.min().as("minEventTime"), stats.jobsFailed.sum().as("jobsFailed"), stats.totalCount.sum().as("totalCount"),

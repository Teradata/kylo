...
CREATE TABLE `thinkbig`.`BATCH_JOB_EXECUTION_CTX_VALS` (
...
CREATE TABLE `thinkbig`.`BATCH_STEP_EXECUTION_CTX_VALS` (
  `JOB_EXECUTION_ID` bigint(20) NOT NULL,
...
CREATE TABLE `thinkbig`.`NIFI_EVENT` (
...
CREATE TABLE `thinkbig`.`NIFI_FEED_PROCESSOR_STATS` (
...
CREATE TABLE `SLA_METRIC_ASSESSMENT` (
  `ID` binary(16) NOT NULL,
  `MESSAGE` varchar(255) DEFAULT NULL,
  `RESULT` varchar(255) DEFAULT NULL,
  `CREATED_TIME` timestamp NULL DEFAULT NULL,
...
CREATE TABLE `SLA_OBLIGATION_ASSESSMENT` (
  `ID` binary(16) NOT NULL,
...
SELECT MAX(JOB_INSTANCE_ID), JOB_EXECUTION_ID
FROM (
SELECT EVENT_ID, FLOW_FILE_ID, COUNT(*)
FROM BATCH_NIFI_JOB
GROUP BY EVENT_ID, FLOW_FILE_ID
HAVING COUNT(*) > 1
)

...
UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(JOB_INSTANCE_ID),0)+1 FROM BATCH_JOB_INSTANCE)
WHERE PK_COLUMN = 'JOB_INSTANCE_ID';

UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(JOB_EXECUTION_ID),0)+1 FROM BATCH_JOB_EXECUTION)
WHERE PK_COLUMN = 'JOB_EXECUTION_ID';

UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(STEP_EXECUTION_ID),0)+1 FROM BATCH_STEP_EXECUTION)

...
DELETE FROM thinkbig.BATCH_NIFI_STEP
...
SELECT event_id,  count(*) FROM thinkbig.BATCH_NIFI_STEP
group by event_id
having count(*) >1) x
...
SELECT STEP_EXECUTION_ID,  count(*) FROM thinkbig.BATCH_NIFI_STEP
...
(`PK_COLUMN`,
`VALUE_COLUMN`)
SELECT 'JOB_INSTANCE_ID', MAX(JOB_INSTANCE_ID)+1 FROM BATCH_JOB_INSTANCE;

INSERT INTO `thinkbig`.`GENERATED_KEYS`
(`PK_COLUMN`,
`VALUE_COLUMN`)
SELECT 'JOB_EXECUTION_ID', MAX(JOB_EXECUTION_ID)+1 FROM BATCH_JOB_EXECUTION;

INSERT INTO `thinkbig`.`GENERATED_KEYS`
(`PK_COLUMN`,
`VALUE_COLUMN`)
SELECT 'STEP_EXECUTION_ID', MAX(STEP_EXECUTION_ID)+1 FROM BATCH_STEP_EXECUTION;

ELSE

UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT MAX(JOB_INSTANCE_ID)+1 FROM BATCH_JOB_INSTANCE)
WHERE PK_COLUMN = 'JOB_INSTANCE_ID';


UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT MAX(JOB_EXECUTION_ID)+1 FROM BATCH_JOB_EXECUTION)
WHERE PK_COLUMN = 'JOB_EXECUTION_ID';

UPDATE `thinkbig`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT MAX(STEP_EXECUTION_ID)+1 FROM BATCH_STEP_EXECUTION)
WHERE PK_COLUMN = 'STEP_EXECUTION_ID';

END IF;


CREATE TABLE IF NOT EXISTS `thinkbig`.`BATCH_JOB_EXECUTION_CTX_VALS` (
...
CREATE TABLE IF NOT EXISTS `thinkbig`.`BATCH_STEP_EXECUTION_CTX_VALS` (
  `JOB_EXECUTION_ID` bigint(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `thinkbig`.`NIFI_EVENT` (
...
CREATE TABLE IF NOT EXISTS `thinkbig`.`NIFI_FEED_PROCESSOR_STATS` (
...
THEN
ALTER TABLE `thinkbig`.`SLA_METRIC_ASSESSMENT`
...
ALTER TABLE `thinkbig`.`BATCH_JOB_EXECUTION_PARAMS`

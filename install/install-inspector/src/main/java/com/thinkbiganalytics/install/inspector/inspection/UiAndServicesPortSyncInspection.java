...
public class UiAndServicesPortSyncInspection extends AbstractInspection {
    ...
    public InspectionStatus inspect(Configuration configuration) {
        ...
        if (!valid) {
            inspectionStatus.setError("'server.port' property in kylo-services/conf/application.properties does not match port number specified by '" + ZUUL_ROUTES_API_URL

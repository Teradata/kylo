...
=== Install NiFi
    ...
    . Option 2: Leverage an existing NiFi instance
        ...
        .. There is a controller service that requires a MySQL database connection. You will need to copy the database driver jar to a location on the NiFI node. The
           pre-defined templates have the default location set to /opt/nifi/mysql.

           1. Create a folder to store the database driver jar in.

           2. SCP the /opt/thinkbig/thinkbig-services/lib/mariadb-java-client-<version>.jar to the folder in step #1
...
=== NiFi Clustering
Additional configuration is needed for multiple instances of Apache NiFi to work together in a cluster. The following changes should be made to /opt/nifi/current/conf/nifi.properties to enable clustering:

    nifi.cluster.is.node=true
    # The address that other nodes should use to connect to this node:
    nifi.cluster.node.address=localhost
    nifi.cluster.node.protocol.port=8078
    # A comma-separated list of ZooKeeper servers for leader elections:

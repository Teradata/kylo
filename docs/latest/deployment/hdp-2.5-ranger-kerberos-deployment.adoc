...
- [ ] Kerberos Principal for "hive" on the KDC Host:
...
. Edit the thinbig-services configuration file
    ...
    nifi.service.think_big_metadata_service.rest_client_url=http://ip-10-0-158-39.us-west-2.compute.internal:8400/proxy/metadata
    ...
    nifi.getjmstopic.url=tcp://<KYLO_EDGE_HOSTNAME>:61616
    nifi.putjms.url=tcp://<KYLO_EDGE_HOSTNAME>:61616
    nifi.indexelasticsearch.hostname=<KYLO_EDGE_HOSTNAME>
...
.. Edit the "Upload to HDFS" and remove "Remote Owner" and "Remote Group"

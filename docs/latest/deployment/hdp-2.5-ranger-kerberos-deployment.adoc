...
. Edit the thinbig-services configuration file
    ...
    config.elasticsearch.hostName=<KYLO_EDGE_HOSTNAME>
    # Set the JMS server hostname for the Kylo hosted JMS server
    config.elasticsearch.jms.url=tcp://<KYLO_EDGE_HOSTNAME>:61616
...
. Update NiFi processors for Kylo template versions prior to 0.5.0
    ...
    We need to update a few settings in the elasticsearch and data ingest template. This is not required for templates in 0.5.0 or greater since they will be set during import

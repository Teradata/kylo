...
public class HiveShellTableMergeSyncSupport extends TableMergeSyncSupport {
    ...
    @Override
    protected java.sql.ResultSet doSelectSQL(Statement st, String sql) throws SQLException {
        List<String> results = shell.executeQuery(sql);
        return new ResultSetWrapper(results);
    }

    static class ResultSetWrapper implements ResultSet {
        ...
        ResultSetWrapper(List<String> values) {
            results = values;
            if (values.size() > 0) {
                columns = values.get(0).split("\\t").length;
            }
        }
        ...
        public String getString(int columnIndex) throws SQLException {
            return results.get(row).split("\\t")[columnIndex - 1];
        }
        ...
        public ResultSetMetaData getMetaData() throws SQLException {
            ...
            Mockito.when(meta.getColumnCount()).thenReturn(columns);
            return meta;
        }

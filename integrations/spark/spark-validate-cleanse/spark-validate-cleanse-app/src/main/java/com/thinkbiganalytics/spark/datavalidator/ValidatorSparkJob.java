 ...
 */
@Component
public class ValidatorSparkJob extends JavaSparkJob {

    private static final Logger log = LoggerFactory.getLogger(ValidatorSparkJob.class);
    /*
    ...
    protected SQLContext getHiveContext() {
        return hiveContext;
    }

    public void doValidate(SparkContext sc) {
        try {

            hiveContext = new HiveContext(sc);
            scs = new SparkContextService16();
            ...
            //log.info("Deployment Mode - " + sc.getConf().get("spark.submit.deployMode"));
            policyMap = new FieldPolicyLoader().loadFieldPolicy(fieldPolicyJsonPath);
    ...
    public Object runJob(SparkContext sc, Config jobConfig) {
        ...
        try {
            ...
            //ValidatorSparkJob app = ctx.getBean(ValidatorSparkJob.class);
            setArguments(jobConfig.getString("targetDatabase"), jobConfig.getString("entity"),
                             jobConfig.getString("partition"), jobConfig.getString("pathToPolicyFile"));
    ...
    public SparkJobValidation validate(SparkContext sc, Config config) {
        ...
        if (config.hasPath("targetDatabase") && config.hasPath("entity") &&
            config.hasPath("partition") && config.hasPath("pathToPolicyFile")) {
            return SparkJobValid$.MODULE$;
        } else {
            return SparkJobInvalid$.MODULE$.apply("Missing one or more arguments <targetDatabase> <entity> <partition> <pathToPolicyFile>");
        }
    }
}
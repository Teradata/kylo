...
object TransformJobTracker16 {

  private val log = LoggerFactory.getLogger(classOf[TransformJobTracker16])
  ...
  override def addSparkListener(engine: SparkScriptEngine): Unit = {
    engine.getSparkContext.addSparkListener(new SparkListener() {
      ...
      override def onJobEnd(@Nonnull event: SparkListenerJobEnd): Unit = {
        TransformJobTracker16.log.trace("Job {} ended", event.jobId)
      ...
      override def onJobStart(@Nonnull event: SparkListenerJobStart) {
        TransformJobTracker16.log.trace("Job {} started with stages {}", event.jobId, event.stageIds)
        ...
        if (job.isEmpty) {
          TransformJobTracker16.log.debug("Missing job {}", groupId)
          return
        }
      ...
      override def onStageCompleted(@Nonnull event: SparkListenerStageCompleted) {
        TransformJobTracker16.log.trace("Stage {} completed with failure {}", event.stageInfo.stageId, event.stageInfo.failureReason)
      ...
      override def onStageSubmitted(@Nonnull event: SparkListenerStageSubmitted) {
        TransformJobTracker16.log.trace("Stage {} submitted with {} tasks", event.stageInfo.stageId, event.stageInfo.numTasks)
      ...
      override def onTaskEnd(@Nonnull event: SparkListenerTaskEnd) {
        TransformJobTracker16.log.trace("Task {} completed for stage {} with {}", event.taskInfo.taskId.toString, event.stageId.toString, event.reason.toString)

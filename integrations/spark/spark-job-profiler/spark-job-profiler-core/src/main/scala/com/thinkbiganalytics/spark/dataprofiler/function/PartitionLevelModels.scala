...
/** Creates a statistics model from RDD values.
  ...
  */
class PartitionLevelModels(val schemaMap: Broadcast[Map[Int, StructField]], val profilerConfiguration: ProfilerConfiguration) extends (Iterator[((Int, Any), Int)] => Iterator[StandardStatisticsModel])
    with Serializable {
  ...
  override def apply(iter: Iterator[((Int, Any), Int)]): Iterator[StandardStatisticsModel] = {
    ...
    for ((k, v) <- iter) {
      statisticsModel.add(k._1, k._2, v.toLong, schemaMap.value(k._1))
    }

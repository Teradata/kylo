 ...
 *
 * <p>Before calling {@link #stopAsync()}, all Spark should should be cancelled to ensure that no
 * additional tables will be created.</p>
 */
public class TransformService extends AbstractScheduledService
{
    ...
    /**
     * Executes the specified transformation and returns the name of the Hive table containing the
     * results.
     *
    ...
    /**
     * Converts the specified transformation request to a Scala script that can be executed by the
     * script engine.
     *
     * @param request the transformation request
    ...
    protected String toScript (@Nonnull final TransformRequest request)
    {
        ...
        script.append("}\n");
        script.append("new Transform(tableName, sqlContext).run()\n");
    ...
    private void updateWeight (@Nonnull final String table, @Nonnull final SQLContext context)
    {
        ...
        List<Row> properties = this.engine.getSQLContext().sql("SHOW TBLPROPERTIES " + identifier)
                .collectAsList();
        int size = -1;
    ...
    static class TableCache implements RemovalListener<String,Integer>, Weigher<String,Integer>
    {
        ...
        /**
         * Iterates over the list of expired tables. The tables are guaranteed to no longer be
         * active but are not guaranteed to exist.
         *
        ...
        /**
         * Associates the specified byte size with the specified table name in this cache,

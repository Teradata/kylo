    ...
    private Date getEarliestOrLatestAlertTimestamp(TIMESTAMP_TYPE timestampType, boolean onlyErrors) {
        ...
        if (alerts != null) {
            for (ServiceAlert alert : alerts) {
                ...
                if (time != null && ((onlyErrors && alert.getState().isError()) || !onlyErrors) && (latestTime == null || (timestampType.equals(TIMESTAMP_TYPE.LATEST) && latestTime != null && time.after(latestTime)) ||
                        (timestampType.equals(TIMESTAMP_TYPE.EARLIEST) && latestTime != null && time.before(latestTime)))) {
                    latestAlert = alert;
                    latestTime = time;
                }
            }

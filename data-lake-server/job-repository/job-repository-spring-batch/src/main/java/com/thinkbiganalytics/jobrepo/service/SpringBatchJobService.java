...
public class SpringBatchJobService extends AbstractJobService {
  ...
  /**
   * Take the given job execution and abandon the job.  To do this in spring batch, there is no native interface.  So We find the
   * job and manually mark the executing step as failed, so it get re-run, and then mark the overall execution as failed.  This
   * will allow it to be restarted.
   *
   * @param executionId The job execution to start again
   */
  @Override

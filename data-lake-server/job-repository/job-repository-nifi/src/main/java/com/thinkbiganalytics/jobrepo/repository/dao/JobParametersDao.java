...
public class JobParametersDao extends AbstractJdbcBatchMetadataDao {
  ...
  private void insertParameter(Long executionId, JobParameter.ParameterType type, String key, Object value, boolean identifying) {
    ...
    if (type == JobParameter.ParameterType.STRING) {
      args =
          new Object[]{executionId, key, type, value, new Timestamp(0L), Long.valueOf(0L), Double.valueOf(0.0D), identifyingFlag};
    ...
    this.getJdbcTemplate().update(this.getQuery(
        "INSERT into %PREFIX%JOB_EXECUTION_PARAMS(JOB_EXECUTION_ID, KEY_NAME, TYPE_CD, STRING_VAL, DATE_VAL, LONG_VAL, DOUBLE_VAL, IDENTIFYING) values (?, ?, ?, ?, ?, ?, ?, ?)"),
                                  args, argTypes);
  }
  ...
  public int updateJobParameter(Long executionId, String key, Object value, JobParameter.ParameterType type) {
    ...
    return this.getJdbcTemplate().update(this.getQuery(
        "UPDATE %PREFIX%JOB_EXECUTION_PARAMS SET TYPE_CD = ?, STRING_VAL = ?, DATE_VAL = ?, LONG_VAL = ?, DOUBLE_VAL = ? WHERE JOB_EXECUTION_ID = ? and  KEY_NAME = ?"),
                                         args, argTypes);
  }

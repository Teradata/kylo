...
public class NifiDao implements InitializingBean {
    ...
    public List<ProvenanceEventRecordDTO> getEvents(Long startEventId, Long endingEventId) {
        ...
        if (startEventId != null && endingEventId != null) {
            query = "SELECT EVENT_ID,JSON FROM " + NIFI_PROVENANCE_EVENT_TABLE + " WHERE EVENT_ID >=" + startEventId + " AND EVENT_ID <=" + endingEventId + " ORDER BY EVENT_ID ";
        } else if (startEventId == null && endingEventId != null) {
            query = "SELECT EVENT_ID,JSON FROM " + NIFI_PROVENANCE_EVENT_TABLE + " WHERE EVENT_ID <=" + endingEventId + " ORDER BY EVENT_ID ";
        } else if (startEventId != null && endingEventId == null) {
            query = "SELECT EVENT_ID,JSON FROM " + NIFI_PROVENANCE_EVENT_TABLE + " WHERE EVENT_ID >=" + startEventId + " ORDER BY EVENT_ID ";
        }

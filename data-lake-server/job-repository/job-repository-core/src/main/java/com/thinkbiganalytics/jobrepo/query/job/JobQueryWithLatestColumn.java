...
public class JobQueryWithLatestColumn extends AbstractConstructedQuery implements JobQueryConstants {
    ...
    protected String getDefaultJoins() {
        ...
        query += " LEFT JOIN BATCH_JOB_EXECUTION_PARAMS jobType on jobType.JOB_EXECUTION_ID = e.JOB_EXECUTION_ID AND jobType.KEY_NAME = '" + FeedConstants.PARAM__JOB_TYPE + "' ";
        query += "LEFT JOIN (select ji.JOB_NAME, MAX(je.JOB_INSTANCE_ID) JOB_INSTANCE_ID, MAX(je.JOB_EXECUTION_ID) JOB_EXECUTION_ID " +

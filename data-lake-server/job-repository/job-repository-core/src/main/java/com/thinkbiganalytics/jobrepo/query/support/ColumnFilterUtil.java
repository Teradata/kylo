...
public class ColumnFilterUtil {
     ...
     * @return
     */
    public static String applySqlFiltersToQuery(String query, ColumnFilter filter, Map<String, Object> namedParameters, String bindParam) {
        int counter = 0;
        ...
        for (ColumnFilterItem aFilter : filtersToApply) {
            if (aFilter.isApplyToWhereClause()) {
                ...
                query += " AND " + tableAlias + aFilter.getSqlConditionBeforeOperator() + " " + ColumnFilterUtil.getSqlBindParamString(aFilter.getOperator(), bindVar) + " ";
    ...
    public static boolean applyDatabaseTypeDateDiffSql(DatabaseType databaseType, ColumnFilter columnFilter) {
        ...
        if (datePart != null) {
            ...
            columnFilter.setSqlString(" AND " + DatabaseQuerySubstitutionFactory.getDatabaseSubstitution(databaseType).getDateDiffWhereClause("e.START_TIME", datePart, filterValue) + " ");
            return true;
        }
        return false;
    }

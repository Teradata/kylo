...
public class FeedHealthQuery extends AbstractConstructedQuery implements FeedQueryConstants {
    ...
    private void setDefaultFilter() {
        ...
        defaultFilter.setSqlString(" AND e.status <> 'ABANDONED' AND (e.status IN('FAILED','UNKNOWN') OR e.EXIT_CODE IN('FAILED'))  ");
    ...
    public QueryBuilder getQueryBuilder() {
        ...
        return newQueryBuilder().select("SELECT count(e.JOB_INSTANCE_ID) CNT, MAX(e.END_TIME) END_TIME,feed.STRING_VAL FEED_NAME, 'UNHEALTHY' HEALTH ")
                .from("BATCH_JOB_EXECUTION e inner join (select job_execution.JOB_INSTANCE_ID, MAX(job_execution.JOB_EXECUTION_ID) JOB_EXECUTION_ID\n" +
                        "                        FROM BATCH_JOB_EXECUTION job_execution\n" +
                        "                        INNER JOIN BATCH_JOB_INSTANCE i on job_execution.JOB_INSTANCE_ID = i.JOB_INSTANCE_ID\n" +
                        "                        GROUP BY job_execution.JOB_INSTANCE_ID) maxe\n" +
                        "                on maxe.JOB_EXECUTION_ID = e.JOB_EXECUTION_ID\n" +
    ...
    public QueryBuilder getQueryBuilderx() {
        return newQueryBuilder().select("SELECT count(ji.JOB_INSTANCE_ID) CNT, MAX(e.END_TIME) END_TIME,feed.STRING_VAL FEED_NAME, CASE WHEN e.status <> 'ABANDONED' AND (e.status IN('FAILED','UNKNOWN') OR e.EXIT_CODE IN('FAILED')) then 'UNHEALTHY' " +
                ...
                .groupBy("feed.STRING_VAL, CASE WHEN e.status <> 'ABANDONED' AND (e.status IN('FAILED','UNKNOWN') OR e.EXIT_CODE IN('FAILED')) then 'UNHEALTHY' else 'HEALTHY' END ");
    ...
    protected String getDefaultJoins() {
        ...
        query += " INNER JOIN BATCH_JOB_EXECUTION_PARAMS jobType on jobType.JOB_EXECUTION_ID = e.JOB_EXECUTION_ID AND jobType.KEY_NAME = '"+ FeedConstants.PARAM__JOB_TYPE+"' AND jobType.STRING_VAL = 'feed' ";

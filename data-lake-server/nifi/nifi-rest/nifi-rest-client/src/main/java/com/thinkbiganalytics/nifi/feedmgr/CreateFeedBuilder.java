...
public class CreateFeedBuilder {
    ...
    private String createProcessGroupForFeed() throws JerseyClientException {
        ...
        if(categoryGroup == null) {
            throw new JerseyClientException("Unable to get or create the Process group for the Category "+category+". Error occurred while creating instance of template "+templateId+" for Feed "+feedName);
        }
        ...
        if(feedGroup != null) {
            try {
                ...
                throw new JerseyClientException("Previous version of the feed "+feedName+" was found.  Error in attempting to version the previous feed.  Please go into Nifi and address any issues with the Feeds Process Group");
            }
        }
    ...
    private FlowSnippetEntity instantiateFlowFromTemplate(String processGroupId) throws JerseyClientException {
        ...
        FlowSnippetEntity response = restClient.postForm("/controller/process-groups/" + processGroupId + "/template-instance", form, FlowSnippetEntity.class);
        return response;
    }
    ...
    private void updateControllerServiceReferences(List<ProcessorDTO> processors) {
        ...
        try {
            ...
            for (final NifiProperty property : properties) {
                ...
                if (StringUtils.isNotBlank(controllerService)) {
                    ...
                    if(!set)
                    {
                        ...
                        List<PropertyDescriptorDTO.AllowableValueDTO> allowableValueDTOs = property.getPropertyDescriptor().getAllowableValues();
                        //if any of the allowable values are enabled already use that and continue
                        List<PropertyDescriptorDTO.AllowableValueDTO> enabledValues = Lists.newArrayList(Iterables.filter(allowableValueDTOs, new Predicate<PropertyDescriptorDTO.AllowableValueDTO>() {
                            @Override
                                ...
                                if(StringUtils.isBlank(controllerServiceName)){
                                  ...
                                  if(allServices.containsKey(allowableValueDTO.getValue()) ) {
                                    ...
                                    try {
                                        ...
                                        if(entity != null && NifiProcessUtil.SERVICE_STATE.ENABLED.name().equalsIgnoreCase(entity.getControllerService().getState())){
                                            controllerServiceSet = true;
                                            break;
                                        }
                                    } catch (JerseyClientException e) {
                        ...
                        if(!controllerServiceSet){
                            errors.add(new NifiError(NifiError.SEVERITY.WARN,"Error trying to enable Controller Service " + controllerServiceName +" on referencing Processor: " + property.getProcessorName() + " and field " + property.getKey() + ". Please go to Nifi and configure and enable this Service before creating this feed.", "Controller Services"));
                        }
          ...
          errors.add(new NifiError(NifiError.SEVERITY.FATAL,"Error trying to identify Controller Services. "+e.getMessage(),"Controller Services"));
        }
    }
    ...
    private void markProcessorsAsRunning(NifiProcessGroup newProcessGroup, List<ProcessorDTO> nonInputProcessors) {
        if(newProcessGroup.isSuccess()) {
            ...
            restClient.markProcessorGroupAsRunning(newProcessGroup.getProcessGroupEntity().getProcessGroup());
                ...
                newProcessGroup.addError(newProcessGroup.getProcessGroupEntity().getProcessGroup().getId(),"",NifiError.SEVERITY.WARN,errorMsg,"Process State");
                newProcessGroup.setSuccess(false);
            }
        }
    }
    ...
    private void markInputAsRunning(NifiProcessGroup newProcessGroup, ProcessorDTO input) {
        try {
            restClient.setInputAsRunningByProcessorMatchingType(newProcessGroup.getProcessGroupEntity().getProcessGroup().getId(), inputProcessorType);
        }catch(JerseyClientException error){
            String errorMsg = "Unable to mark group as "+ NifiProcessUtil.PROCESS_STATE.RUNNING+" for "+input.getName()+ "("+inputProcessorType+").";
            newProcessGroup.addError(newProcessGroup.getProcessGroupEntity().getProcessGroup().getId(),input.getId(), NifiError.SEVERITY.WARN,errorMsg,"Process State");
            newProcessGroup.setSuccess(false);
    ...
    private void updateFeedSchedule(NifiProcessGroup newProcessGroup, ProcessorDTO input) {
        if(feedSchedule != null) {
            ...
            try {
                ...
                String errorMsg = "Unable set Scheduling Information for feed "+input.getName()+" on "+input.getType()+". "+e.getMessage();

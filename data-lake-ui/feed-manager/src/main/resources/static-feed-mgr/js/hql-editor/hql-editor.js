            ...
            compile:function(element,attrs) {
                return {
                    pre: function preLink($scope, iElement, iAttrs, controller) {
                        if($scope.scrollResults == true) {
                            var browseResults = iElement.find('.browse-results');
                            browseResults.attr('overflow-scroll', '');
                            browseResults.attr('parent', 'md-dialog-content');
                            browseResults.attr('offset-height', "120");
                        }
                    },
...
this.loadingHiveSchemas = false;
        ...
        function getTable(schema,table){
            ...
            var successFn = function (response) {
               var tableColumns = response.data;
                ...
                $mdToast.show(
                    $mdToast.simple()
                        .textContent('Table and Field autocomplete is now available')
                        .hideDelay(3000)
                );
                */
    ...
    angular.module(MODULE_FEED_MGR)
        .directive('thinkbigHqlEditor', directive);

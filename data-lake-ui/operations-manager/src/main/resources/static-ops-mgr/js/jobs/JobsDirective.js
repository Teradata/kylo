...
(function () {
    ...
    var controller = function ($scope,$http, $stateParams, $interval, $timeout, $q,JobData, TableOptionsService, PaginationDataService, AlertsService, StateService, IconService, TabService) {
     ...
     //   loadJobs();
        ...
        function transformJob(job){
            ...
            if(!shouldRefresh ) {
                $timeout(function() {triggerJobActionListener("updateEnd",job);},10);
            }
            ...
            if(shouldRefresh) {
                self.timeoutMap[instanceId] = $timeout(function(){getRunningJobExecutionData(instanceId,executionId)},1000);
            }
    ...
    angular.module(MODULE_OPERATIONS).controller('JobsCardController', controller);

    angular.module(MODULE_OPERATIONS)
        .directive('tbaJobs', directive);
